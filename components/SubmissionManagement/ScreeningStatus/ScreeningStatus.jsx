...
export default function ScreeningStatus(props) {
  ...
  return (
    <button onClick={() => onShowDetails(submissionId)}>
      <div className={`screening-status ${setClassName()}`}>
        {/* status */}
        {hasStatus && !hasPending ?
          <span className={`status ${setStatusClassName()}`}>
            {hasStatus.substring(0, hasStatus.indexOf('ed'))}</span> :
          <span className="has-pending">Not yet performed</span>}{/* pending */}
        {/* warning */}
        {hasWarnings && <span className="warning">
